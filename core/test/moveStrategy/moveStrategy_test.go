...
func Test(t *testing.T) {
    ...
    g.Describe ("Move strategy for Ai Player", func(){
     g.It("ai TIE MOVE: should make move(8) as best move on current board state", func() {
         ...
         o, _ := e.MakeMove(6, "X")
         q, _ := o.MakeMove(7, "O")
         g.Assert(moveStrategy.AiMove(q, p1, p2)).Equal(8)
       })
     g.It("ai BLOCK MOVE: should make move(7) as best move on current board state", func() {
         ...
         e, _ := k.MakeMove(5, "X")
         o, _ := e.MakeMove(6, "X")
         g.Assert(moveStrategy.AiMove(o, p1, p2)).Equal(7)
       })
        g.It("ai WIN MOVE: should make move(2) as best move on current board state", func() {
         ...
         g.Assert(moveStrategy.AiMove(e, p1, p2)).Equal(2)
       })
       g.It("ai BLOCK MOVE: should make move(8) as best move on current board state", func() {
         ...
         g.Assert(moveStrategy.AiMove(e, p1, p2)).Equal(8)
       })
      g.It("ai WIN MOVE: should make move(6) as best move on current board state", func() {
         ...
         e, _ := d.MakeMove(4, "X")
         f, _ := e.MakeMove(3, "O")
         g.Assert(moveStrategy.AiMove(f, p1, p2)).Equal(6)
       })
      ...
      g.It("ai BLOCK MOVE: should make move(7) as best move on current board state", func() {
         ...
         e, _ := d.MakeMove(4, "X")
         f, _ := e.MakeMove(5, "O")
         g.Assert(moveStrategy.AiMove(f, p1, p2)).Equal(7)
       })
       g.It("ai BLOCK MOVE: should make move(7) as best move on current board state", func() {
         ...
         f, _ := e.MakeMove(3, "O")
         k, _ := f.MakeMove(6, "X")
         g.Assert(moveStrategy.AiMove(k, p1, p2)).Equal(7)
       })
      g.It("ai WIN MOVE: should make move(4) as best move on current board state", func() {
         ...
         f, _ := e.MakeMove(5, "X")
         k, _ := f.MakeMove(6, "O")
         g.Assert(moveStrategy.AiMove(k, p1, p2)).Equal(4)
       })
     g.It("ai BLOCK MOVE: should make move(7) as best move on current board state", func() {
         ...
         d, _ := i.MakeMove(2, "O")
         e, _ := d.MakeMove(4, "X")
         g.Assert(moveStrategy.AiMove(e, p1, p2)).Equal(7)
       })
     g.It("ai WIN MOVE: should make move(5) as best move on current board state", func() {
         ...
         k, _ := d.MakeMove(7, "X")
         e, _ := k.MakeMove(4, "O")
         g.Assert(moveStrategy.AiMove(e, p1, p2)).Equal(5)
       })
     g.It("ai WIN MOVE: should make move(6) as best move on current board state", func() {
         ...
         k, _ := d.MakeMove(4, "O")
         e, _ := k.MakeMove(8, "X")
         g.Assert(moveStrategy.AiMove(e, p1, p2)).Equal(6)
       })
